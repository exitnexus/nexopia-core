...
=end
	...
	class Privilege
		...
		def self.load_all
			@@account_privileges = $site.cache.get("account_privileges", 60){
		...
		def has?(mod, bit)
			...
			if (@@account_privileges[@userid])
				case bit
				when Symbol, String
					privilege = [mod.typeid, bit.to_s]
					if (@@account_privileges[@userid].include?(privilege))
						return true
					end
				when Array
					privilege = [mod.typeid, nil]
					bit.each {|b|
						privilege[1] = b.to_s
						if (@@account_privileges[@userid].include?(privilege))
							return true
