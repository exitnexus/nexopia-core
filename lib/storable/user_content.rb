...
module UserContent
	...
	def self.allowed_html
		@allowed_html = @allowed_html || {
			"a" => {"href" => /^(https?:\/\/|mailto:|\/).{1,500}$/, "target" => /^_new$/, "class" => /^(body)?\s*(user_content)?/, "rel" => /^(nofollow)$/},
			"img" => {"src" => /^(https?:\/\/|\/).{1,500}$/, "minion_name" => /^(user_content_image)?$/, "url" => /^(https?:\/\/|\/).{1,500}$/, "width" => /^[0-9]{1,3}%?$/, "height" => /^[0-9]{1,3}%?$/, "border" => /^0$/},
			"font" => {"size" => /^[1-7]$/, "color" => /^((#([a-f]|[A-F]|[0-9]){3}(([a-f]|[A-F]|[0-9]){3})?)|[a-zA-Z]+)$/, "face" => /.{1,24}/},
			"b" => {},
			"i" => {},
			"u" => {},
			"sub" => {},
			...
			"em" => {},
			"strong" => {},
			"div" => {"style" => /^(text\-align:(center|left|right|justify)|visibility:hidden)$/, "class" => /^(quote)$/, "align" => /^center$/, "width" => /^[0-9]{1,3}$/, "height" => /^[0-9]{1,3}$/},
			"span" => {"style" => /^font\-size:\s*[0-9]{1,3}(?:px|pt);?$/},
			"link" => {"rel" => /^(image_src|video_src)$/, "href" => /^(https?:\/\/|\/).{1,500}$/}
		}
		return @allowed_html
	end
	...
	# filter_all: if true, any html tags will be filtered out, including "allowed" html
	# 						if false, "allowed" html won't be filtered out
	def self.html_filter(string, filter_all=false)
		...
		begin
			...
			# From my experience, it reaches here when the attribute buffer of hpricot is full - as in you have an element
			# with more than 16K of attributes.
			$log.warning "KNOWN ISSUE NEX-1986: Hpricot threw an exception: #{e.to_s}"
			$log.warning "User HTML was: #{string}"
		...
		do_node = lambda {|elem, level_str|
			...
			when Hpricot::Elem
				valid = allowed_html[elem.name] if !filter_all
