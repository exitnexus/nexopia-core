lib_require :Core, "storable/storable", "users/user", "users/anonymous_user", "secure_form", "info_messages"
...
class Session < Cacheable
	...
	def Session.get(sessionkey)
		...
		begin
			...
			if (
				(!session.cachedlogin && (session.activetime < (time - $site.config.session_timeout))) ||
				(
					#ignore the lock ip check on dev because the way we do hosting over VPN sometimes screws it up (particularly via the flash uploader)
					(session.lockip.to_i() && $site.config.environment != :dev) && 
					(request_ip.to_i() & 0xFFFFFF00) != (session.ip.to_i() & 0xFFFFFF00)
				)
			)
				expired_cookie = session.destroy(request_ip);
				if(!expired_cookie.nil?())
			...
			if(session.user.nil?())
				$log.warning("Session was found with nil user(#{session.userid}). May happen after a user delete.");
				return nil;
			end
	...
	def Session.build(remote_addr, userid, cachedlogin)
		...
		if (cachedlogin)
			expire = Time.now + $site.config.long_session_timeout if cachedlogin
		else
			expire = nil
		end
		key = makeRandKey();
	...
	def destroy(remote_addr)		
		...
		elsif( cookie.kind_of?(Array) )
			# Occationally the cookie is an array and usually it's empty.
			# Don't know why this happens, but we deal with it here.
			return nil if (cookie.empty? )
			key, cookie_userid = cookie.first.scan(/[^:]+/);
		else
	...
	def site_skin_value(site_skin_value_name)
		site_skin_value_name = site_skin_value_name.to_s if site_skin_value_name.kind_of?(Symbol)
		return SkinMediator.request_skin_value(:Nexoskel, user.skin, site_skin_value_name)
	end
