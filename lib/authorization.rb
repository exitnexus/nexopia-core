...
class Authorization
	...
	def activate(user, key, check_expire = true)
		if (user.nil?)
		...
		if (email.key == key.strip)
			if ( check_expire && (email.time < (Time.now.to_i - WEEK_SECONDS)) )
			...
			$log.warning "User #{user.userid} supplied activation key '#{key}', which did not match stored key of '#{email.key}'."
