lib_require :Core, "typesafehash", "bufferio", "session", "user_time", "pagehandler", 'url', 'info_messages', 'mimetype';
lib_require :Trackback, 'trackbackmodule'
...
class PageRequest
	...
	def initialize(method, area, uri, headers, cookies, params, user, reply = nil)
		begin
			if (PageRequest.current.nil?)
				$page_id = rand(89999)+10000 #always be exactly 5 digits
			end
#			SystemTimer.timeout_after(1800) do
				...
				begin
					$log.trace(LogStartRequest.new(prev_req, self), :pagehandler);
					...
					$log.trace(LogEndRequest.new(prev_req, self, (endtime-starttime)*1000), :pagehandler);
				end
			end
#		rescue Timeout::Error
#			$log.error "PageRequest Timeout::Error, uri: #{uri}, userid: #{user == nil ? "none" : user.userid}"
#		end
	end
	...
	def PageRequest.convert_to_nested_hash(cgi_hash)
			...
			while (key =~ /(.*)\[(.+)?\]$/)
				...
				if ($2)
					...
					if (!new_hash[key].kind_of? Array)
						$log.error "Tried to add '#{key}' as an array and a value."
					end
					old_vals = [*value];
					old_vals.each{|v|
						new_hash[key] << v
		...
		return TrackBack::extract_conversions(new_hash)
	end
	...
	def PageRequest.new_from_cgi(cgi)
		if (!(['GET','POST','HEAD'].include? cgi.env_table['REQUEST_METHOD']))
			...
			# 		return self;
			# 	end
			$log.error "#{cgi.env_table['REQUEST_METHOD']} requests are not supported yet."
		...
		reply = PageReply.new(cgi.stdoutput, true, accepted_encodings, true)
...
require 'yaml'
	...
	# Prevents traversal of the object properties, including request_stack
	# This method is automatically called during exception handling in errorlog
	def inspect
		self.to_s
	end
	...
	def negotiate_content_type(*types)
		...
		if (accept_headers)
			...
			accept_headers.each {|choice|
				...
				$log.trace("Client accepts type #{choice[0]} at quality #{choice[1]}", :pagehandler);
				accept_choices[choice[0]] = choice[1];
			}
			best = nil;
			types.each {|type|
				$log.trace("Negotiating type #{type}...", :pagehandler);
				if (quality = accept_choices[type]) #  || accept_choices['*/*'] removed because IE doesn't give it a quality and never says it can take text/html.
					$log.trace("Type #{type} possible with quality #{quality}", :pagehandler);
					if (!best || best[1] < quality)
						$log.trace("Type #{type} is the best so far.", :pagehandler);
						best = [type, quality];
					end
				end
			}
	...
	def extension_content_type(filename)
		return MimeType.file(filename).to_s
	end
end
...
class PageReply
	...
	attr_reader :title
	# The meta info for this page.  Hash table made up of name => content.
	attr_reader :meta
	...
	def initialize(out = nil, mix_headers = true, supported_encodings = [], add_info_messages=false)
		if(out.nil?())
			out = StringIO.new();
		end
		...
		out.initialize_buffer {
			if (@headers['Content-Type'] =~ /^text\// && !@headers['Content-Encoding'])
				#XXX: This has been hacked in here because of the way output buffering works.
				#It was nearly impossible to determine when a page request was done but had not
				#yet written it's buffer out. We needed to do this here rather than in the skeleton
				#so that we could catch handle's as well.
				#A better approach would be to change most of our handles to page :Ajax or some other
				#such construct so that we can hook extra data into them sanely but no one knows how
				#to do that right now.
				#If this is putting stuff places it's not the easiest fix is likely to add more conditions to
				#this if statement.
				if (add_info_messages && !headers['X-info-messages-off'] && (status.to_s !~ /3.*/))
					PageRequest.current.session.write_info_messages!
				end
	...
	def set_title(title, include_nexopia = true)
		@title = title
		@title = "Nexopia | " + @title if include_nexopia
		return @title
	end
	
	# Add the meta information for the page.
	...
	def set_cookie(cookie)
		if (cookie.nil?)
			$log.warning "Tried to set a cookie which was nil."
		else
