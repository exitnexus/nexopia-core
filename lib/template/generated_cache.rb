...
#   representations of the filename being different.
# - UPDATE: Lies!!! Dirty, dirty lies!!! Subdirectories have come to the codebase! Muhahahaha!
# 					(at least for layout, script, and template directories)
...
class GeneratedCodeCache
	...
	def self.check_dependencies(ruby_file, file, lib_build_time)
		...
		dependencies.split("=")[1].split(",").each{|dependency|
			if (dependency.size > 0)
				...
				$log.trace "Checking dependency '#{dependency}'", :template
				if !File.exists?(ruby_dep) or !check_valid(ruby_dep, source_file, lib_build_time)
					$log.trace "Failed.", :template
					return false;
				end
			end
		}
	...
	def self.get_cached(*args)
		...
		$log.trace("Looking for #{ruby_file}", :template);
	...
	def self.check_cached_file(source_file, ruby_file, lib_build_time)
		if check_valid(ruby_file, source_file, lib_build_time) and check_dependencies(ruby_file, source_file, lib_build_time)
			$log.trace("Found usable #{ruby_file}", :template);
			return true;
		else
			$log.trace("Found old #{ruby_file}", :template);
	...
	def self.load_template(source_file, source_descriptor, lib_build_time)
		...
		if (check_cached_file(source_file, ruby_file, lib_build_time))
			begin
				$log.trace("Requiring... #{ruby_file}", :template);
				require(ruby_file);
			rescue SyntaxError => e
				$log.error "In file: #{ruby_file}";
				$log.error $!;
	...
	def self.load_templates()
		$log.trace("Searching Modules for template files.", :template);
			...
			source_dirs(mod).each{|dir|
				find_templates(dir) {|file|
					if(match = file.match(source_regexp))
						...
						#preload if needed
						if(($site.config.environment != :dev) && !found)
							begin
								instance(SiteModuleBase.directory_name(mod.name), template_name)
							rescue
								$log.error "#{SiteModuleBase.directory_name(mod.name)} - #{template_name} pre-loading failed"
								$log.error $!
							end
						end
					end
