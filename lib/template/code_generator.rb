...
class CodeGenerator
	...
	# Append a print statement for the input paramenter, which is a variable or method call
	# such as "object.to_s"
	...
	def generate(xml_file_name)
		...
		@exports.each{ |name,dump|
			file = AtomicFile::create("#{basename}.#{name}.robj") { |out|
				out.write dump;
			}
...
#{@code}
		...
		AtomicFile::create(File.basename(filename), $site.config.generated_base_dir) { |f|
			$log.trace("Writing #{@class_name}", :template);
		...
		begin
			$log.trace("Requiring #{@class_name}", :template);
				load("#{$site.config.generated_base_dir}/#{File.basename(filename)}");
		rescue Exception => e
			$log.error "#{$!}:#{$!.backtrace}";
		end

		$log.trace("Done.", :template);
		#@code.html_dump;
		#puts @code;
	end
