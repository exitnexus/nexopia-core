...
module Template
	...
	class DefaultView < Processor
		...
		class << self
				...
				code.append "if(#{obj})\n"
					if(!iter && !index)
						...
						if(index || node.attributes['join'] || node.attributes['alternating'])
							...
							handle_script_generation(node, code);
			...
			def handle_script_generation(node, code)
				max_fixnum = 1073741823
				if (node.attribute("t:json"))
					code.append("_json_id = #{node.attributes["t:json"]}.object_id.to_s(36)+rand(#{max_fixnum}).to_s(36)\n")
				...
				if (node.attribute("t:available"))
					if (node.attribute("id"))
						code.append("\t\t_available_id = '#{node.attributes['id']}'\n");
					else
						code.append("\t\t_available_id = \"\#{rand(#{max_fixnum}).to_s(36)}\#{rand(#{max_fixnum}).to_s(36)}\"\n");
						node.attributes["id"] = "\#{_available_id}"
					end
					code.append_output("\"<script>YAHOO.util.Event.onContentReady('\#{_available_id}', function(){Overlord.minionAvailable('\#{_available_id}');});</script>\"")
				end
			end
				...
				@@attributes.each{ | key,fn_handle |
					...
					if (arg)
						if (code.get_var_type(Template::get_literal_var(arg.value)) == type)
							...
							$log.trace("#{type} doesn't match #{arg.value} - #{var_r}.", :template);
						end
						handled = true;
					end
				}
...
=end
			...
			def translate_button(node, code, &block)
				set_button_id = ""
				node.prefix = ""
				node.attributes.each {|name,value|
					...
					if (name != 't:available' && name != 't:json')
...
=end
			...
			def pp_time(time)
				now_time = UserTime.at(Time.now)
				now = now_time.to_i				

				if (time.to_i > (now - Constants::DAY_IN_SECONDS))
					since = now - time.to_i
					case since
					when (0...Constants::MINUTE_IN_SECONDS)
						message = "moments"
					when (Constants::MINUTE_IN_SECONDS...Constants::HOUR_IN_SECONDS)
						message = "#{(since/Constants::MINUTE_IN_SECONDS).to_i} minute"
						message += 's' if (since >= (2 * Constants::MINUTE_IN_SECONDS)) # ie at least two minutes
					when (Constants::HOUR_IN_SECONDS...Constants::DAY_IN_SECONDS)
						message = "#{(since/Constants::HOUR_IN_SECONDS).to_i} hour"
						message += 's' if ( since >= (2 * Constants::HOUR_IN_SECONDS) ) # ie at least two hours
					end
					message += " ago"
					return message
				else
					now_date = Date.new(now_time.year, now_time.month, now_time.day)
					compare_date = Date.new(time.year, time.month, time.day)
					days_apart = (now_date - compare_date).to_i
			...
			def translate_info_messages(node, code)
				__typeid = node.attribute("t:typeid")
				__div_class = node.attribute("class")
				
				if(!__typeid.nil?)
					div_default_class = "info_messages"
				else
					div_default_class = "top_level_info_messages"
				end
				
				div_string = "<div id='info_messages' class='#{div_default_class} #{__div_class}' minion_name='info_messages' "
				if(!__typeid.nil?)
					div_string = div_string + "message_type='#{__typeid}'"
				end
				div_string = div_string + "></div>"
				
				code.append_print(div_string)
			end
