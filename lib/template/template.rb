...
module Template
	...
	class Cache < GeneratedCodeCache
		...
		def self.class_name(_module, file_base)
			"#{prefix}_#{_module.to_s.upcase}_#{file_base.upcase.gsub('/','_DIR_')}"
		end

		def self.output_file(_module, file_base)
			"#{$site.config.generated_base_dir}/#{prefix}_#{_module.to_s.upcase}_#{file_base.upcase.gsub('/','_DIR_')}.gen.rb"
		end
	...
	# Create a new Template class from the template file 'tem_file' in the
	# module 'mod_file', if necessary.  The result of this call is to parse
	# and generate the class, if necessary, and add it to the local cache of
	# templates.
	...
	def Template.get_name(mod_file, tem_file)
		name = Cache.prefix + "_" + mod_file.to_s.upcase + "_" + tem_file.gsub('/','_DIR_').gsub(/[^a-zA-Z_0-9]/, "_").upcase;
	end
	...
	public; def Template.instance(mod_file, template_name)
		...
		if !Cache.instantiatedClasses[[mod_file,template_name]]
			...
			$log.info("Generated #{template_name} in #{'%.4f' % duration} seconds", :template);
		elsif ($site.config.environment == :dev) #all templates are pre-loaded live, so don't try to create it
			#get_file_path(mod_file, tem_file)
			$log.trace("Checking cache... #{mod_file}:#{template_name}", :template);
		...
		$log.trace("Instantiating... #{template_name}", :template);
	...
	public; def Template.get_class(mod_file, tem_file)
		if !Cache.instantiatedClasses[[mod_file,tem_file]]
			...
			duration = Time.now.to_f - start_time
			$log.trace("Generated #{tem_file} in #{'%.4f' % duration} seconds", :template);
		end
	...
	class TemplateClass
		...
		private; def initialize(class_name, source_name, symbol, source)
			...
			@vars = Hash.new;
			filename = "#{$site.config.generated_base_dir}/#{class_name}.gen.rb"
				...
				if (element.kind_of? REXML::Text)
					...
					if element.to_s =~ /^\[if[0-9A-Za-z ]+\]>.*<!\[endif\]$/ then
						...
						if $site.config.environment == :dev
							code.append_print "<!--" + element.to_s + "-->"
						end
					end
				else
					yield element;
		...
		public; def self.handle_node(element, code, &block)
			...
			Processor.for_namespace(ns).send(:"translate_#{element.name.gsub('-', '_').downcase}", element, code) { |node|
				@depth = (@depth || 0) + 1;
				#$log.trace(' ' * @depth + "Entering node of type #{element.name}.", :template);
				#code.push_position doc.positions[element];
				yield node;
				#code.pop_position
				#$log.trace(' ' * @depth + "Leaving node of type #{element.name}.", :template);
				@depth = @depth - 1;
			};
