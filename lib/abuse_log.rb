...
class AbuseLog < Storable
	...
	if (site_module_loaded?(:UserDump))
		...
		def self.user_dump(uid, start=0, finish=Time.now.to_i)
			query = "SELECT * FROM abuselog WHERE userid = ? AND time BETWEEN ? AND ? ORDER BY time"
			result = $site.dbs[:moddb].query(query, uid, start, finish)

			abuse_report = StringIO.new
			result.each {|row|
				action = ACTIONS[row['action'].to_i]
				date_reported = Time.at(row['time'].to_i).strftime("%a %b %d %H:%M:%S %Z %Y")
				reason = REASONS[row['reason'].to_i]
...
"			}
	...
	ABUSE_ACTION_DELETE_SHOUT = 23;
	ABUSE_ACTION_CHANGE_USERNAME = 24;
	...
	ACTIONS = {
		...
		ABUSE_ACTION_DELETE_SHOUT => 'Delete Shout',
		ABUSE_ACTION_CHANGE_USERNAME => 'Change Username',
	}
