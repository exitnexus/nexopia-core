...
class Mysql
  ...
  def read_one_row(field_count)
    data = read
    if data[0] == 254 and (data.length == 1 or data.length == 5) ## EOF
      return
    end
    rec = []
  ...
  def skip_result()
    if @status == :STATUS_USE_RESULT then
      loop do
        data = read
        break if data[0] == 254 and (data.length == 1 or data.length == 5) ## EOF
      end
      @status = :STATUS_READY
    end
  end
