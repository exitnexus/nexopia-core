...
class TestPages < PageHandler
	declare_handlers("test_stuff") {
		...
		page :GetRequest, :Full, :notification_test, "notification_test"
		handle :PostRequest, :notification_test_post, "notification_test_post"
		handle :PostRequest, :notification_test_post2, "notification_test_post2"
		handle :PostRequest, :notification_test_post3, "notification_test_post3"
	...
	def notification_test
		t = Template::instance("core", "notification_test")
		puts t.display
	end
	
	def notification_test_post
		request.session.notify_error :title => "BLAH", :message => "ajax test message!"
	end

	def notification_test_post2
		request.session.notify_error :title => "BLAH", :message => "anOther ErRor mESSage", :typeid => 2
		request.session.notify_error :title => "BLAH", :message => "anOther ErRor mESSage type 3", :typeid => 3
		request.session.notify_error :title => "BLAH", :message => "MORE ERRORZ!", :typeid => 2
		site_redirect( url / :test_stuff / :notification_test )
	end

	def notification_test_post3
		request.session.notify_success :title => "SUCCESS? I THINK SO.", :message => "Congrats! You deserve a sticker.", :typeid => 2

		site_redirect( url / :test_stuff / :notification_test )
	end
	
	def test_self()
		$log.info("ran the test page...")
...
=end
	...
	def sean_test
		...
		request.skeleton.smilies.each do |key, value|
			emoticons << "<td valign=\"middle\"><img src=\"#{$site.static_file_url("smilies/#{value}.gif")}\" alt=\"#{key}\" /></td>"
		end
		t.emoticons = "<div style=\"display: none;\">
		<table id=\"emoticons_list\" height=\"30\" width=\"100%\"><tr>#{emoticons}</tr></table>
		<img id=\"emoticon_left_arrow_img\" src=\"#{$site.static_file_url("core/enhanced_text_input/images/icon_left_arrow.gif")}\" />
		<img id=\"emoticon_right_arrow_img\" src=\"#{$site.static_file_url("core/enhanced_text_input/images/icon_right_arrow.gif")}\" />
		<img id=\"emoticon_left_back_img\" src=\"#{$site.static_file_url("core/enhanced_text_input/images/left_arrow_back.png")}\" />
		<img id=\"emoticon_right_back_img\" src=\"#{$site.static_file_url("core/enhanced_text_input/images/right_arrow_back.png")}\" />
		</div>"
