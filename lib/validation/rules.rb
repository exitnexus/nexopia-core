lib_require :core, 'banned_users', 'date_utils'
lib_require :core, 'users/deleted_user', 'users/user';
lib_require :core, 'validation/results', 'validation/rule', 'validation/value_accessor';

lib_want :Badwords, 'badwords';
	...
	class Rules
		...
		class CheckUsernameAvailable < Rule
			...
			def validate
				...
				# validate that username is not used
				unless (UserName.by_name(@username).nil?)
					return Results.new(:error, "Username already in use")
				end

				# validate that username is not reseved
				unless (UserNameReserve.by_name(@username).nil?)
					return Results.new(:warning, "Username reserved")
				end
		...
		class CheckChecked < Rule
			def initialize(checked,error_msg=nil)
				super([checked], [error_msg]);
		...
		class CheckLegalUsername < Rule
			...
			def validate
				...
				if (@username.length < $site.config.min_username_length)
					...
					return Results.new(:error, "Username too long.");
				end
				
				if (site_module_loaded?(:Badwords))
					if (!Badwords::scan(@username, UserName::typeid).empty?)
						return Results.new(:error, "Username contains banned word");
					end
				end
		...
		class CheckTerms < Rule
			...
			def _calculate_age(dob)
				return DateUtils::years_between(dob)
			end
		end
			...
			def validate
				...
				if (@year == -1 || @month == -1 || @day == -1)
					...
					begin
						...
						# probably user managed to get a date before 1901-12-14 into the field.						
						return Results.new(:error, "Invalid Date of Birth");
					end
					...
					if(age < 13)
						...
						return Results.new(:error, "Must be less than 70");
					else
						return Results.new(:valid, "");
					end
				end
